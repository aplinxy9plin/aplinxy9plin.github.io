...
function scrollHandler(){
    ...
    if (top > 200 && !document.body.classList.contains("displayMode"))
    {
    ...
    if (top <= 200 && document.body.classList.contains("displayMode"))
    {
        document.body.classList.remove("displayMode");
    }
/*
    var howDoes = document.querySelector("#howDoesItworkAR");

    if (howDoes)
    {
        var xy = getPos(howDoes);
        if (top > xy.y + 150)
        {
            howDoes.classList.add("fixed");
        }
        else
        {
            howDoes.classList.remove("fixed");
        }
    }
*/
}
